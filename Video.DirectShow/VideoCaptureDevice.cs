...
namespace AForge.Video.DirectShow
{
    ...
    public class VideoCaptureDevice : IVideoSource
    {
        ...
        /// </summary>
        /// <param name="TVFormat"></param>
        /// <returns>eturns true on success or false otherwise.</returns>
        ///   
        /// <exception cref="ArgumentException">Video source is not specified - device moniker is not set.</exception>
        /// <exception cref="ApplicationException">Failed creating device object for moniker.</exception>
        /// <exception cref="NotSupportedException">The video source does not support camera control.</exception>
        ///
        public bool GetAnalogFormat(out string TVFormat)
        {
            bool ret = true;
            TVFormat = string.Empty;
            // check if source was set
            if ((deviceMoniker == null) || (string.IsNullOrEmpty(deviceMoniker)))
            {
                throw new ArgumentException("Video source is not specified.");
            ...
            lock (sync)
            {
                ...
                if (!(tempSourceObject is IAMAnalogVideoDecoder))
                {
                    throw new NotSupportedException("The video source does not support camera control.");
                }

                IAMAnalogVideoDecoder pCamControl = (IAMAnalogVideoDecoder)tempSourceObject;
                AnalogVideoStandard tvformat=AnalogVideoStandard.NTSC_M;
                int hr = pCamControl.get_TVFormat(out tvformat);
                TVFormat = tvformat.ToString();
                ret = (hr >= 0);
        ...
        /// </summary>
        /// <param name="tvformat">The emun in the internals Structures.cs AnalogVideoStandard</param>
        /// <returns>eturns true on success or false otherwise.</returns>
        ///   
        /// <exception cref="ArgumentException">Video source is not specified - device moniker is not set.</exception>
        /// <exception cref="ApplicationException">Failed creating device object for moniker.</exception>
        /// <exception cref="NotSupportedException">The video source does not support camera control.</exception>
        ///
        public bool SetAnalogFormat(long tvformat)
        {
            bool ret = true;
            if ((deviceMoniker == null) || (string.IsNullOrEmpty(deviceMoniker)))
            {
            ...
            lock (sync)
            {
                ...
                IAMAnalogVideoDecoder pCamControl = (IAMAnalogVideoDecoder)tempSourceObject;
                int hr = pCamControl.put_TVFormat( (AnalogVideoStandard)tvformat);
                ret = (hr >= 0);
